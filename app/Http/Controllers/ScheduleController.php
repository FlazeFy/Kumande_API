...
class ScheduleController extends Controller
{
    ...
    public function updateScheduleData(Request $request, $id){
        $sch = Schedule::where('schedule_id', $id)->update([
    ...
    public function createSchedule(Request $request){
        $check = Schedule::select('schedule_id','schedule_time')
        ...
        function getFirstId($schedule){
            ...
            foreach($schedule as $sc){
                $before_alph = substr($sc->schedule_id,0,2);
                $before_num = substr($sc->schedule_id,2,1);
